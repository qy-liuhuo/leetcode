...
//</ul>
//
//<div><div>Related Topics</div><div><li>栈</li><li>设计</li><li>数据流</li><li>单调栈</li></div></div><br><div><li>👍 478</li><li>👎 0</li></div>
...
class StockSpanner {
    ...
    class Node{
        public int price;
        public int preCnt;
        public Node(int price, int preCnt){
            this.price = price;
            this.preCnt = preCnt;
        }
    }
    ...
    public int next(int price) {
        if (dq.isEmpty()){
            dq.offerLast(new Node(price, 1));
            return 1;
        }else{
            int cnt = 1;
            while (!dq.isEmpty() && dq.peekLast().price <= price){
                cnt += dq.pollLast().preCnt;
            }
            dq.offerLast(new Node(price, cnt));
            return cnt;
        }
    }
}
